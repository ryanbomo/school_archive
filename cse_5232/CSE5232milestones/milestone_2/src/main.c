...
void runTCP(int portNum, char dbfilepath[]){
  ...
  while( (cli_socket = accept (serv_socket, ( struct sockaddr*) &cli_addr, &cli_addr_size))){
    ...
    while( (msglen= recv(cli_socket, buffer, MAXLINE,0))>0){
      ...
      write(cli_socket, returnMessage, strlen(returnMessage));
...
char * add_gossip(char message[], char dbfilepath[]){
  ...
  broadcastTCP(text_mes, dbfilepath);
