...
void *do_server(void *args){
  ...
  // stuct to send info to time out checker thread
  struct checkerLoopParams *checkLoopParameters;
  checkLoopParameters = malloc(sizeof(*checkLoopParameters));
  checkLoopParameters->_time_out = time_out;
  checkLoopParameters->database = db_add;
  ...
  pthread_create(&time_check, NULL, checker_loop,checkLoopParameters);
    
  // don't exit while child threads are still listening
  pthread_exit(NULL);
}

// function to check table for users who have passed time_out
// This runs on a separate thread, so that it can check to drop
// whenever it gets scheduled.  It can get interupted and not perform
// check for exactly current results, but it is approximate enough
// to desired behavior.
void *checker_loop(void *args){
  // get data from struct passed in
  struct checkerLoopParams *checker_stuff = args;
  int time_out = checker_stuff->_time_out;
  char *dbfilepath = checker_stuff->database;
  int rc, tockcount;
  sqlite3 *db;
  char *sql_temp = "DELETE FROM PEERS WHERE (TICK_COUNT<= ";
  char *sql_1, sql;
 ...
 WHERE (HolidayDate >= '1/1/2011')
  ...
  while(1){
    ...
    tockcount = GetTickCount();
    tockcount = tockcount -time_out;
    char s[11]; 
    sprintf(s,"%ld", tockcount);
    sql_1 = concat(sql_temp,s);
    sql = concat(sql_1, ")");
...
void *listen_TCP(void *args){
  ...
  while(1){
    ...
    // get IP address, update tick time on db
      struct sockaddr_in* pV4Addr = (struct sockaddr_in*)&cli_addr;
      struct in_addr ipAddr = pV4Addr->sin_addr;
      char ip_str[INET_ADDRSTRLEN];
      inet_ntop( AF_INET, &ipAddr, ip_str, INET_ADDRSTRLEN );
      update_peer_tick(dbfilepath, ip_str);
      
      struct timeval timeout;      	// set 20 second timeout for TCP
      timeout.tv_sec = 20;		
      timeout.tv_usec = 0;
...
// updates peer tick/tock time on the database
// used to judge whether the peer should be kept or now on the now third thread
void update_peer_tick(char* db, char* address){
  char *sql_1 = "UPDATE PEERS SET TICK_COUNT =";
  char *sql_2 = " WHERE IP = '";
  char *sql_3 = "'";
  char *some, *place, *holder, *strings;
  int rc;
  
  int tockcount = GetTickCount();
  char s[11]; 
  sprintf(s,"%ld", tockcount);
  some = concat(sql_1,s);
  place = concat(some,sql_2);
  holder = concat(place,address);
  strings = concat(holder,sql_3);
    rc = sqlite3_open(dbfilepath,&db);
  
  rc = sqlite3_exec(db,strings,callback, 0, &zErrMsge);
  if( rc!=SQLITE_OK ){
    fprintf(stderr, "SQL error: %s\n", zErrMsge);
    sqlite3_free(zErrMsge);
    }
